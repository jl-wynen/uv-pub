...
pub type ForkSet = ForkMap<()>;

/// A map from package names to their values for a given fork.
#[derive(Debug, Clone)]
pub struct ForkMap<T>(FxHashMap<PackageName, Vec<Entry<T>>>);
...
impl<T> ForkMap<T> {
    ...
    pub fn add(&mut self, requirement: &Requirement, value: T) {
        let entry = Entry {
            value,
    ...
    /// given fork.
    pub fn contains(&self, package_name: &PackageName, env: &ResolverEnvironment) -> bool {
        !self.get(package_name, env).is_empty()
    }

    /// Returns `true` if the map contains any values for a package.
    pub fn contains_key(&self, package_name: &PackageName) -> bool {
    ...
    pub fn get(&self, package_name: &PackageName, env: &ResolverEnvironment) -> Vec<&T> {
